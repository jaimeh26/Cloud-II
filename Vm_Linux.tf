...
resource "azurerm_linux_web_app" "mcit_linux" {
  name                = "${var.prefix}linux-web-app-${keys(azurerm_virtual_network.mcitvn)[count.index]}"
  resource_group_name = azurerm_resource_group.mcit420zz5um.name
  location            = azurerm_resource_group.mcit420zz5um.location
  service_plan_id     = azurerm_service_plan.mcitsp.id
  virtual_network_subnet_id = local.subnet_ids[keys(azurerm_virtual_network.mcitvn)[count.index]]
